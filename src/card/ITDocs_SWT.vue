...
<template>
  <v-card>
    ...
    <!-- <v-btn
              ...
              @keyup.enter="uploaditdocs()"
              ><v-icon color="blue">mdi-</v-icon>ອັບໂຫຼດ</v-btn
            > -->
    <!-- <v-btn
              color="primary"
              text
    ...
    <v-card style="font-family: phetsarath OT">
      ...
      <v-card class="scrollmenu">
        <v-card-title>
          ...
          &nbsp;
          <v-btn
            @click="checkUsercms = true"
            color="primary"
            text
            v-on:click="getUserCMS()"
            outlined
            style="font-size: 12px; height: 30px; width: 100px"
            ><v-icon color="blue">mdi-</v-icon>ກວດ USER CMS</v-btn
          >
...
export default {
  ...
  data() {
    return {
      ...
      headerusercms: [
        ...
        { text: "NOTED", value: "STATUS" },
        { text: "NEW PASSWORD", value: "REPONSE_NEWPASS" },
  ...
  mounted() {
    ...
    // setInterval(this.fetchDataFromServer, 3000000);
  },
  ...
  methods: {
    ...
    uploaditdocs() {
      if (
        ...
        axios
          .post("api/update/uploadITDOCS", formData, {
            headers: {
    ...
    checkupdates_imageurl() {
      if (this.selectedFile_image != "") {
        ...
        axios
          .post("api/post/postgethistory_ITdocsURL", {
    ...
    validateEditdataInfo() {
      if (this.selectedFile_image != "") {
        ...
        // this.display_image = true;
        axios
          .post("api/post/postgetInfo_ITdocs", {
            itdocs_id: this.selectedFile_image,
            headers: {
              "Content-Type": "multipart/form-data",
            },
    ...
    conformupdateInfo() {
      ...
      if (
        ...
        axios
          .post("api/update/updateITDOcsINFO", {
    ...
    updateusercms() {
      if (this.passcms !== "" && this.cmsid !== "") {
        axios
          .post("api/update/updatedcmsuser", {
    ...
    validatedelete() {
      if (this.selectedFile_image != "") {
        //
        axios
          .post("api/delete/deleteitdocsid", {
            itdocs_id: this.selectedFile_image,
