<template>
  <v-card class="MobileDbaView" style="font-family: Saysettha OT">
    <v-card-title
      ...
      "
    >
      <v-icon style="color: chocolate">mdi-</v-icon>ທຸລະກໍາ AUTHTXN ຜ່ານຊ່ອງທາງ
      LDB Terminal by Laps card PROD Only 2023 ~ LAPS-CHANNEL</v-card-title
    >
    <!-- <v-divider></v-divider> -->
    <v-card>
      <v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "LAPS_TXN", value: "LAPS_TXN" },
        { text: "LAPS_FEE", value: "LAPS_FEE" },
        { text: "LAPS_AMT", value: "LAPS_AMT" },
  ...
  mounted() {
    this.isLoading = "red";
    axios
      .get("http://10.0.22.236:9889/api/get/getTxn_fromLasp")
      .then(({ data }) => {
        this.getdata = data;
