<template>
  <v-card class="MobileDbaView" style="font-family: Saysettha OT">
    <v-card-title
      ...
      "
    >
      <v-icon style="color: chocolate">mdi-</v-icon>ທຸລະກໍາ AUTHTXN ຜ່ານຊ່ອງທາງ OVERSEA
      PROD Only 2023 ~ International</v-card-title
    >
    <!-- <v-divider></v-divider> -->
    <v-card>
      <v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      { text: "AUTHTXN_REQUEST_DATE", value: "AUTHTXN_REQUEST_DATE"},
        { text: "OVERSEA_CARD_TXN", value: "OVERSEA_CARD_TXN" },
        { text: "OVERSEA_CARD_FEE", value: "OVERSEA_CARD_FEE"},
        { text: "OVERSEA_CARD_AMT", value: "OVERSEA_CARD_AMT" },
  ...
  mounted() {
    this.isLoading = "red";
    axios
      .get("http://10.0.22.236:9889/api/get/getTxn_fromOversea_Channel")
      .then(({ data }) => {
        this.getdata = data;
